## About 

This is my own theme edit of [Etterna](https://github.com/etternagame/etterna)'s theme "Til Death", the goal of this repository is to document the changes/fixes i make while giving out an public release of the theme 

This theme **is only playable on 0.74.4** and in a ratio of 16:9.

## Credits

- Poco0137
- Darkmans39
- Steffen for the grade counter logic
- Sidek for the particule system of xoon3
- MISSILE228 for some graphic stuff (mainly the main menu artwork)
- Reimuboobs for the original theme

- the team who made the [ManipFactor Addon](https://github.com/MaidOfFire/ManipFactorEtterna/tree/main):
- Opakyl
- U1wknUzeU6 
- karatovv

## Screenshots for a Preview 

<img src="https://raw.githubusercontent.com/ItzFireable/unreadable-rain-edit/main/Graphics/screenshotsexample/1.png"/>
<img src="https://raw.githubusercontent.com/ItzFireable/unreadable-rain-edit/main/Graphics/screenshotsexample/2.png"/>
<img src="https://raw.githubusercontent.com/ItzFireable/unreadable-rain-edit/main/Graphics/screenshotsexample/3.png"/>
<img src="https://raw.githubusercontent.com/ItzFireable/unreadable-rain-edit/main/Graphics/screenshotsexample/4.png"/>


## TODO:
- rework the screenevaluationscreen again
- rework the profile tab
- work on giving the user more customization without lua shenanigans
- implement more ideas, or qols, or maybe more paritys from rebirth