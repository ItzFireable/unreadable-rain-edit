...
function ResultScreenStyle()
	local t = {
		Name = "ResultScreenStyle",
		LayoutType = "ShowAllInRow",
		SelectType = "SelectOne",
		OneChoiceForAllPlayers = false,
		ExportOnChange = true,
        ExportOnCancel = false,
		Choices = {"Unredable Rain", "ReimuBoobs Legacy", "Til' Death"},
		LoadSelections = function(self, list, pn)
			local pref = themeConfig:get_data().global.ResultScreenStyle
			if pref == 1 then
				list[1] = true
			elseif pref == 2 then
				list[2] = true
		...
		SaveSelections = function(self, list, pn)
			...
			themeConfig:get_data().global.ResultScreenStyle = value
			themeConfig:set_dirty()
			themeConfig:save()
		end
	}
